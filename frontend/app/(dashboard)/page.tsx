...
import { getOverview, getMonthlyMetrics, getTopProducts, getTopCustomers } from "@/lib/services/dashboard-service"
...
export default function Home() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        setLoading(true)
        const [overviewData, monthlyData, productsData, customersData] = await Promise.all([
          getOverview(),
          getMonthlyMetrics(),
          getTopProducts(),
          getTopCustomers(),
    ...
    <>
      <DashboardShell className="mb-1">
        ...
        <p className="text-muted-foreground">
          Welcome, {user?.name}! Here's an overview of your inventory management system.
        </p>
      </DashboardShell>
        ...
        <>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{overview?.monthly_orders || 0}</div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">${formatCurrency(overview?.monthly_revenue || 0)}</div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{overview?.active_customers || 0}</div>
                <p className="text-xs text-muted-foreground">Active customers this month</p>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <CardContent>
                <div className="text-2xl font-bold">{overview?.low_stock_items || 0}</div>
              ...
              <CardContent className="pl-2">
                <Overview
                  data={monthlyMetrics.map((metric) => ({
                    name: new Date(metric.month).toLocaleDateString("en-US", { month: "short" }),
